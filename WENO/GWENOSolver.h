#ifndef TRIANGULAR_SOLVERS_GWENOLF_H
#define TRIANGULAR_SOLVERS_GWENOLF_H
...
namespace euler
{
    ...
    private:
        ...
        std::array<std::array<int, 9>, 6> const polynomial_groups =
                {
                        std::array<int, 9>{1, 3, 4, 2, 5, 6, 0, 7, 8},
                        std::array<int, 9>{2, 3, 4, 1, 7, 8, 0, 5, 6},
                        std::array<int, 9>{2, 5, 6, 0, 7, 8, 1, 3, 4},
                        std::array<int, 9>{0, 5, 6, 2, 3, 4, 1, 7, 8},
                        std::array<int, 9>{0, 7, 8, 1, 3, 4, 2, 5, 6},
                        std::array<int, 9>{1, 7, 8, 0, 5, 6, 2, 3, 4}
                };


    public:

        explicit GWENOSolver(std::vector<Zone> const &constraints,
    ...
    template<class T>
    inline double GWENOSolver<T>::CalculateKsiAverage(Triangle const *pTriangle, double x_0, double y_0, double h) const
    {
    ...
    template<class T>
    inline double GWENOSolver<T>::CalculateEtaAverage(Triangle const *pTriangle, double x_0, double y_0, double h) const
    {
        double area = pTriangle->getArea2D();
        auto const x_1 = pTriangle->getCorner(0)->x();
        auto const y_1 = pTriangle->getCorner(0)->y();
    ...
    template<class T>
    inline double GWENOSolver<T>::CalculateKsiSquareAverage(Triangle const *pTriangle,
    ...
    template<class T>
    inline double GWENOSolver<T>::CalculateEtaSquareAverage(Triangle const *pTriangle,
                                                           double x_0, double y_0, double h) const
    {
        double area = pTriangle->getArea2D();
        auto const x_1 = pTriangle->getCorner(0)->x();
    ...
    template<class T>
    inline double GWENOSolver<T>::CalculateKsiEtaAverage(Triangle const *pTriangle, double x_0, double y_0, double h) const
    {
    ...
    template<class T>
    inline void GWENOSolver<T>::CreateBoundingMesh()
    {
...
//						virtualTriangles[i]->SetIndex(T::m_boundingTriangles.size());
    ...
    template<class T>
    inline void GWENOSolver<T>::GetStencil(Triangle const* pTriangle, std::array<Triangle const*, 10> & stencil) const
    {
        stencil[0] = pTriangle;
    ...
    template<class T>
    inline void GWENOSolver<T>::Init(std::function<std::array<double, 4>(GEOM_FADE2D::Point2)> const &initStateFunction)
    {
...
#pragma omp parallel for
    ...
    template<class T>
    inline void GWENOSolver<T>::GetTriangleReconstructionData(TriangleReconstructionData &trRecData, Triangle const *pTriangle)
    {
...
/*			arma::mat B(6, 10);
    ...
    template<class T>
    inline void GWENOSolver<T>::GetSmoothIndicatorData(TriangleReconstructionData &trRecData, Triangle const *pTriangle) const
    {
        std::array<Triangle const*, 10> stencil;
        GetStencil(pTriangle, stencil);
    ...
    template<class T>
    inline Vec4 GWENOSolver<T>::Reconstruct(Vec4 const& qVec, Triangle const *pTriangle,
                                           Point2 const &gaussianPoint, int edgeNumber) const
    {
        ...
        for(int k = 0; k < 3; ++k)
        {
            ...
            FOReconstructionPolynomial const& fo_pol_2_data = triangleRecData.fo_polynomial[pol_2];
            ...
            smoothIndicator =  arma::square(
                      triangleRecData.smoothIndicatorData[pol_0].alpha[0] * q[fo_pol_0_data.stencil[0]]
                    + triangleRecData.smoothIndicatorData[pol_0].alpha[1] * q[fo_pol_0_data.stencil[1]]
                    + triangleRecData.smoothIndicatorData[pol_0].alpha[2] * q[fo_pol_0_data.stencil[2]]

                    + triangleRecData.smoothIndicatorData[pol_1].alpha[0] * q[fo_pol_1_data.stencil[0]]
                    + triangleRecData.smoothIndicatorData[pol_1].alpha[1] * q[fo_pol_1_data.stencil[1]]
                    + triangleRecData.smoothIndicatorData[pol_1].alpha[2] * q[fo_pol_1_data.stencil[2]]

                    + triangleRecData.smoothIndicatorData[pol_2].alpha[0] * q[fo_pol_2_data.stencil[0]]
                    + triangleRecData.smoothIndicatorData[pol_2].alpha[1] * q[fo_pol_2_data.stencil[1]]
                    + triangleRecData.smoothIndicatorData[pol_2].alpha[2] * q[fo_pol_2_data.stencil[2]]) +
                    arma::square(
                            triangleRecData.smoothIndicatorData[pol_0].beta[0] * q[fo_pol_0_data.stencil[0]]
                            + triangleRecData.smoothIndicatorData[pol_0].beta[1] * q[fo_pol_0_data.stencil[1]]
                            + triangleRecData.smoothIndicatorData[pol_0].beta[2] * q[fo_pol_0_data.stencil[2]]

                            + triangleRecData.smoothIndicatorData[pol_1].beta[0] * q[fo_pol_1_data.stencil[0]]
                            + triangleRecData.smoothIndicatorData[pol_1].beta[1] * q[fo_pol_1_data.stencil[1]]
                            + triangleRecData.smoothIndicatorData[pol_1].beta[2] * q[fo_pol_1_data.stencil[2]]

                            + triangleRecData.smoothIndicatorData[pol_2].beta[0] * q[fo_pol_2_data.stencil[0]]
                            + triangleRecData.smoothIndicatorData[pol_2].beta[1] * q[fo_pol_2_data.stencil[1]]
                            + triangleRecData.smoothIndicatorData[pol_2].beta[2] * q[fo_pol_2_data.stencil[2]]);

            omega_waved[k] = Vec4{gamma_waved, gamma_waved, gamma_waved, gamma_waved}
                             / arma::square(eps + smoothIndicator);
            omega_waved_sum += omega_waved[k];
...
#endif //TRIANGULAR_SOLVERS_GWENOLF_H
