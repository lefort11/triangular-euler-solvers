...
Vec4 HYBRIDSolver::CalculateFlux(Vec4 const &qVec, int triangleNumber, int edgeNumber) const
{
	...
	Vec4 flux_weno{0.0, 0.0, 0.0, 0.0};
	Vec4 flux_cd{0.0, 0.0, 0.0, 0.0};
	...
	Vec4 qVec_plus{0.0, 0.0, 0.0, 0.0};
	...
	for (int gPointNum = 0; gPointNum < 2; ++gPointNum)
	{
		...
		//****************** Forming flux_weno vectors ********************************//
		//Calculating F_minus
		auto const F_minus = Vec4{
...
//		auto const eta_vl_3 = std::max((vel_n_x_plus + c_plus) - (vel_n_x_minus + c_minus), 0.0);
		...
		Vec4 const f = 0.5 * (normal[0] * (F_minus + F_plus) + normal[1] * (G_minus + G_plus));

		flux_weno += f;
/*		for (int k = 0; k < 4; ++k)
        {
            flux_weno -= 0.5 * std::fabs(lambdas_n_waved[k]) * arma::dot(l_n[k], q_plus - q_minus) * r_n[k];
        }

*/
		flux_cd += f;

		flux_weno -= 0.5 * (std::fabs(lambdas_n_waved[0]) * arma::dot(l_n[0], q_plus - q_minus) * r_n[0]
	...
	double const alpha = Alpha(m_triangles[triangleNumber]);

	return (1 - alpha) * 0.5 * flux_weno + alpha * 0.5 * flux_cd;
}